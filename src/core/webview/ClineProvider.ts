	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	...
	constructor(
		...
		this.taskCreationCallback = (instance: Task) => {
			...
			const onTaskCompleted = (taskId: string, tokenUsage: TokenUsage, toolUsage: unknown) =>
	...
	override on<K extends keyof TaskProviderEvents>(
		...
		return super.on(event, listener as (...args: TaskProviderEvents[K]) => void | Promise<void>)
	}
	...
	override off<K extends keyof TaskProviderEvents>(
		...
		return super.off(event, listener as (...args: TaskProviderEvents[K]) => void | Promise<void>)
	}
	...
	async performPreparationTasks(cline: Task): Promise<void> {
	...
	*/
	...
	public async handleModeSwitch(newMode: Mode): Promise<void> {
		...
		if (task) {
			...
			try {
				...
				// <<<< MERGED: start of upstream content
				const historyUpstream = this.getGlobalState("taskHistory") ?? []
				const taskHistoryItemUpstream = historyUpstream.find((item) => item.id === task.taskId)
				// >>>> MERGED: end of upstream content
	...
	// kilocode_change:
	async handleKiloCodeCallback(token: string): Promise<void> {
	...
	// kilocode_change start
	async postRulesDataToWebview(): Promise<void> {
	...
	async fetchMcpMarketplace(forceRefresh: boolean = false): Promise<void> {
		try {
...
Here is the project's README to help you get started:\n\n${mcpDetails.readmeContent}\n${mcpDetails.llmsInstallationContent}`
	...
	async toggleTaskFavorite(id: string): Promise<void> {
		const history = await this.getTaskHistory()
	...
	async setTaskFileNotFound(id: string): Promise<void> {
		const history = await this.getTaskHistory()
		const updatedHistory = history.map((item) => {
			if (item.id === id) {
